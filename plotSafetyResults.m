function plotSafetyResults(time, x_safe, x_unsafe, u_safe, u_unsafe, x_ref, q_v_max, u_max, P, h_min, h_max, savePath)
    ...
    %% 1. 2D TRAJECTORY (X-Y Plane) — diagnostic
    figure('Name', '2D Trajectory', 'Color', 'w', 'Position', [50 50 800 600]);
    plot(x_unsafe(1,:), x_unsafe(2,:), '--', 'Color', c2, 'DisplayName', 'Unsafe'); hold on;
    plot(x_safe(1,:),   x_safe(2,:),   '-',  'Color', c1, 'LineWidth', 2, 'DisplayName', 'Safe');
    if size(x_ref, 2) > 1
        plot(x_ref(1,:), x_ref(2,:), '-.', 'Color', lc, 'DisplayName', 'Reference');
    else
        plot(x_ref(1), x_ref(2), 'p', 'MarkerSize', 12, 'MarkerFaceColor', 'y', ...
            'MarkerEdgeColor', 'k', 'DisplayName', 'Goal');
    end
    plot(x_safe(1,1), x_safe(2,1), 'o', 'MarkerSize', 10, ...
        'MarkerFaceColor', [0.2 0.7 0.2], 'MarkerEdgeColor', 'k', 'DisplayName', 'Start');
    grid on; axis equal;
    xlabel('$x$ (m)', 'Interpreter', 'latex');
    ylabel('$y$ (m)', 'Interpreter', 'latex');
    legend('Location', 'best', 'Interpreter', 'latex');

    %% 2. QUATERNION CONSTRAINT ||q_v|| — diagnostic
    figure('Name', 'Quaternion Constraint', 'Color', 'w', 'Position', [100 100 800 500]);
    plot(time, qv_norm_unsafe, '--', 'Color', c2, 'DisplayName', 'Unsafe $\|\mathbf{q}_v\|$'); hold on;
    plot(time, qv_norm_safe,   '-',  'Color', c1, 'DisplayName', 'Safe $\|\mathbf{q}_v\|$');
    yline(q_v_max, '--', 'Color', lc, 'LineWidth', 2, ...
        'DisplayName', ['$\bar{q}_v = ' num2str(q_v_max, '%.3f') '$']);
    xlabel('Time (s)');
    ...
    %% 3. CONTROL INPUTS — diagnostic
    figure('Name', 'Control Inputs', 'Color', 'w', 'Position', [150 150 600 800]);
    tiledlayout(3, 1, 'TileSpacing', 'compact', 'Padding', 'compact');
    tile_labels = {'$\tau_1$ (Roll)', '$\tau_2$ (Pitch)', '$\tau_3$ (Yaw)'};
    for i = 1:3
        nexttile;
        plot(time, u_unsafe(i,:), '--', 'Color', c2, 'DisplayName', 'Unsafe'); hold on;
        plot(time, u_safe(i,:),   '-',  'Color', c1, 'DisplayName', 'Safe');
        yline( u_max(i), '--', 'Color', lc);
        yline(-u_max(i), '--', 'Color', lc);
        ylabel('N$\cdot$m', 'Interpreter', 'latex');
        title(tile_labels{i}, 'Interpreter', 'latex');
        ylim([-u_max(i)*1.5, u_max(i)*1.5]);
        xlim([time(1), time(end)]);
        grid on;
        if i == 1
            legend('Location', 'best', 'Interpreter', 'latex');
    ...
    %% 4. INVARIANT SET & INTERVENTION — diagnostic
    figure('Name', 'Safety Logic', 'Color', 'w', 'Position', [200 200 800 600]);
    subplot(2,1,1);
    plot(time, Lyap_safe, '-', 'Color', c4, 'LineWidth', 2);
    yline(h_max, '--', 'Color', lc, 'LineWidth', 1.5, 'Label', ['$h_{\max}=' num2str(h_max) '$'], 'Interpreter', 'latex');
    yline(h_min, ':',  'Color', lc, 'LineWidth', 1.5, 'Label', ['$h_{\min}=' num2str(h_min) '$'], 'Interpreter', 'latex');
    ylabel('$V(\mathbf{z})$', 'Interpreter', 'latex');
    ylim([0, h_max * 1.5]);
    xlim([time(1), time(end)]);
    grid on;

    subplot(2,1,2);
    area(time, alpha_monitor, 'FaceColor', [0.8, 0.9, 1.0], 'EdgeColor', c1);
    ...
    %% 5. SECTOR BOUND VERIFICATION — diagnostic
    figure('Name', 'Sector Bound Verification', 'Color', 'w', 'Position', [150 150 1200 400]);
    subplot(1,2,1);
    plot(time, gamma * omega_norm_safe, '--', 'Color', c2, 'DisplayName', '$\gamma\|\omega\|$'); hold on;
    plot(time, phi_norm_safe,           '-',  'Color', c1, 'DisplayName', '$\|\phi(\mathbf{z})\|$');
    xlabel('Time (s)');
    ylabel('Magnitude');
    xlim([time(1), time(end)]);
    legend('Location', 'best', 'Interpreter', 'latex');
    ...
    subplot(1,2,2);
    plot(time, sector_bound_ratio, '-', 'Color', c1, ...
        'DisplayName', '$\|\phi\| / (\gamma\|\omega\|)$'); hold on;
    yline(1.0, '--', 'Color', lc, 'LineWidth', 2, 'DisplayName', 'Bound');
    xlabel('Time (s)');
    ylabel('Ratio');
    ylim([0, 1.25]);
    xlim([time(1), time(end)]);
