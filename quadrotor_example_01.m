...
% kappa_0 must be large enough that kappa_0 * h_0 dominates |h_0_dot| in
% the safe set, otherwise h_1 = h_0_dot + kappa_0*h_0 can become negative
% inside the safe set.  Slack relaxation allows graceful degradation
% under input saturation (constraint violated but u stays bounded).
cbf_opts = struct();
...
plotSafetyResults(time, x_safe, x_unsafe, x_cbf, u_safe, u_unsafe, u_cbf, ...
                       ...
                       'results/quadrotor_example_01');

%% Comparison summary
fprintf('\n=== Comparison Summary (Scenario 1: small goal) ===\n');
...
% First constraint violation time for the CBF baseline
